    ...
    {   
       ...
       foreach (GameObject obj in objectsWithTag)
        ...
        for (int i = 0; i < ROW; i++)
        {
         ...
         List<KeyValuePair<int, int>> sign = new List<KeyValuePair<int, int>>()
            {
        ...
        bool posi =checkSolutionExist(cur.Key,cur.Value);
        if(!posi){
            yield break;
        }
        ...
        private bool checkSolutionExist(int endx, int endy)
    {
        Queue<KeyValuePair<int, int>> q = new Queue<KeyValuePair<int, int>>();
        for(int i=0;i<8;i++){
            for(int j=0;j<8;j++){
                valueExtractor[i,j]=-1;
            }
        }
        int[,] vis = new int[8, 8]
        {
        ...
        {   
            ...
            for(int i=0;i<size;i++)
            {
                ...
                valueExtractor[cur.Key, cur.Value] = layer;
